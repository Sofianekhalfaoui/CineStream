...
export default function TVShows() {
  ...
  // Dynamic slide states corresponding to Top 5 TV Shows of Today
  ...
  const getGenreName = (id: number) => {
    const map: Record<number, { ar: string; en: string; fr: string }> = {
      10759: { ar: 'أكشن ومغامرة', en: 'Action & Adventure', fr: 'Action & Aventure' },
      18: { ar: 'دراما', en: 'Drama', fr: 'Drame' },
      10765: { ar: 'خيال علمي وخيال', en: 'Sci-Fi & Fantasy', fr: 'Sci-Fi & Fantaisie' },
      80: { ar: 'جريمة', en: 'Crime', fr: 'Crime' },
      16: { ar: 'أنميشن', en: 'Animation', fr: 'Animation' },
  ...
  const getFetchUrl = (pageNum: number, genre: string, timeRange = activeTimeRange) => {
    let baseUrl = requests.fetchTVShows;
    let params = `&page=${pageNum}`;
    
    // Apply filters matching requested options:
    if (timeRange === 'newest') {
      params += `&sort_by=first_air_date.desc&first_air_date.lte=${new Date().toISOString().split('T')[0]}`;
    } else if (timeRange === 'top_rated') {
      params += `&sort_by=vote_average.desc&vote_count.gte=50`;
      ...
      params += `&first_air_date_year=${currentYear}&sort_by=popularity.desc`;
    } else if (genre !== 'all') {
      params += `&sort_by=popularity.desc`;
    }

    if (genre === 'turkish') {
      baseUrl = requests.fetchTurkishTV;
    } else {
      let mappedGenre = genre;
      if (genre === '28' || genre === '12') mappedGenre = '10759';
      if (genre === '27') mappedGenre = '9648';
      
      if (genre !== 'all') {
        baseUrl = `${requests.fetchTVShows}&with_genres=${mappedGenre}`;
      }
    }
  ...
  const fetchGenreGridShows = async (pageNum: number, isInitial = false, genre = activeGenre, timeRange = activeTimeRange) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    const activeShow = trendingToday[currentIndex];
    ...
    async function loadTrailer() {
      try {
        const videos = await fetchMovieVideos(activeShow.id, 'tv', selectedLang);
        ...
        if (isSubscribed) {
          if (ytTrailer?.key) {
            ...
            const fallbackVideos = await fetchMovieVideos(activeShow.id, 'tv', 'en-US');
        ...
        console.error('Could not fetch tv section slide video:', e);
  ...
  const activeShowSlide = trendingToday[currentIndex];
  ...
  const handleGenreChange = (genreId: string) => {
    ...
    if (genreId !== 'all') {
      fetchGenreGridShows(1, true, genreId, activeTimeRange);
    }
  };
  ...
  const handleTimeRangeChange = (rangeId: string) => {
    ...
    if (activeGenre !== 'all') {
      fetchGenreGridShows(1, true, activeGenre, rangeId);
    }
  };
  ...
  const genres = [
    ...
    { id: '10765', name: isRTL ? 'خيال علمي' : 'Sci-Fi' },
      ...
      <div className="min-h-screen bg-[#070b19] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-primary font-black uppercase tracking-[0.3em] text-xs">
            {isRTL ? 'جاري تحميل واجهة المسلسلات...' : 'Rendering Series Hub...'}
          </span>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#070b19] text-white overflow-x-hidden relative">
      ...
      {/* 1. DYNAMIC CINEMATIC HERO SLIDERS */}
        ...
        >
            ...
            <AnimatePresence>
              {(!showVideo || !trailerKey) && (
                <motion.img
                  ...
                  src={getImageUrl(activeShowSlide.backdrop_path, 'original')}
                  alt={activeShowSlide.name || activeShowSlide.title}
          ...
          <div className="relative z-20 w-full max-w-4xl mx-auto px-4 pb-3 md:pb-4 flex flex-col items-center text-center space-y-1.5 pointer-events-none">
            ...
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-white tracking-tighter italic uppercase leading-none max-w-3xl drop-shadow-[0_8px_8px_rgba(0,0,0,0.8)]">
              {activeShowSlide.name || activeShowSlide.title}
            </h1>
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 md:gap-3 text-xs font-bold text-gray-300 pointer-events-auto">
              ...
              <span className="text-[11px] md:text-xs">
                • {activeShowSlide.genre_ids?.[0] ? getGenreName(activeShowSlide.genre_ids[0]) : ''}
              </span>
              <span className="text-[11px] md:text-xs">
                • {activeShowSlide.first_air_date?.split('-')[0] || activeShowSlide.release_date?.split('-')[0]}
              </span>
              {activeShowSlide.vote_average > 0 && (
                <span className="flex items-center gap-1 text-yellow-500 font-mono text-xs font-semibold ml-1">
                  <Star className="w-3.5 h-3.5 fill-yellow-500 text-yellow-500" />
                  {activeShowSlide.vote_average.toFixed(1)}
                </span>
              )}
            </div>

            {(() => {
              const favorited = isFavorite(activeShowSlide.id);
              return (
                <>
                  <div className="flex items-center justify-center gap-3 md:gap-4 pointer-events-auto">
                    <button 
                      onClick={() => setSelectedMovie(activeShowSlide)}
                    ...
                    <button 
                      onClick={() => toggleFavorite(activeShowSlide)}
                      className={cn(
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="space-y-1">
              <h1 className="text-3xl md:text-6xl font-black text-white tracking-tighter uppercase leading-none italic">
                {genres.find(g => g.id === activeGenre)?.name} <span className="text-primary">Series</span>
              </h1>
      ...
      {/* 3. MULTI-ROW CINEMATIC LANES (If activeGenre is 'all') */}
      {activeGenre === 'all' ? (
        <div className="space-y-2 md:space-y-4 pb-20">
          
          <MovieRow 
            title={isRTL ? "أفضل 10 مسلسلات اليوم" : "Top 10 Series Today"} 
            fetchUrl={fetchTrendingTVToday} 
            isLargeRow 
            isTop10 
          ...
          <MovieRow 
            title={isRTL ? "مسلسلات شهيرة" : "Popular Series"} 
            fetchUrl={fetchPopularTV} 
          />

          <MovieRow 
            title={isRTL ? "أفضل المسلسلات تقييماً" : "Top Rated Series"} 
            fetchUrl={fetchTopRatedTV} 
          />

          <MovieRow 
            title={isRTL ? "مسلسلات تركية" : "Turkish Series"} 
            fetchUrl={requests.fetchTurkishTV} 
          />

          <MovieRow 
            title={isRTL ? "خيال علمي وخيال" : "Sci-Fi & Fantasy Series"} 
            fetchUrl={fetchSciFiTV} 
          />

          <MovieRow 
            title={isRTL ? "مسلسلات أنمي" : "Anime Series"} 
            fetchUrl={requests.fetchAnime} 
          />

          <MovieRow 
            title={isRTL ? "أكشن ومغامرات" : "Action & Adventure Series"} 
            fetchUrl={fetchActionTV} 
          />

        </div>
      ) : (
        /* 4. FLAT GRID LAYOUT FOR SELECTED CATEGORIES */
        <div id="shows-genre-grid-wrapper" className="max-w-7xl mx-auto px-4 md:px-12 pb-24 z-10 relative">
          
          {shows.length === 0 && !loading && (
            <div className="py-24 text-center">
              <span className="text-gray-500 font-bold uppercase tracking-widest text-sm">
                {isRTL ? 'لا تتوفر مسلسلات في هذه الفئة حالياً' : 'No series found in this selection.'}
              </span>
            </div>
          )}
            ...
            <div className="mt-20 text-center space-y-6">
              ...
              <button
                onClick={() => {
                  ...
                  setPage(nextPage);
                  fetchGenreGridShows(nextPage);
                }}
                disabled={loading}
                className="group relative px-12 py-4 bg-white hover:bg-primary text-[#070b19] hover:text-white rounded-xl font-bold text-xs uppercase tracking-widest transition-all duration-300 disabled:opacity-40 font-black"
              >
                <span className="relative z-10 flex items-center justify-center gap-2">
                  {loading && <Loader2 className="w-4 h-4 animate-spin" />}
                  <span>{isRTL ? 'تحميل المزيد من المسلسلات' : 'Load More Series'}</span>
                </span>
              </button>
            </div>
          )}
